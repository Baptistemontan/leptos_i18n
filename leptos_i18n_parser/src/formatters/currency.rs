...
impl FormatterToTokens for CurrencyFormatter {
    fn view_bounds(&self) -> TokenStream {
        quote!(__l_i18n_crate::__private::NumberFormatterInputFn)
    }
    fn to_view(&self, key: &syn::Ident, locale_field: &syn::Ident) -> TokenStream {
        let Self(width, code) = self;
        quote!(__l_i18n_crate::__private::format_currency_to_view(#locale_field, #key, #width, #code))
    }

    fn fmt_bounds(&self) -> TokenStream {
        quote!(__l_i18n_crate::__private::IntoFixedDecimal)
    }

    fn to_impl_display(&self, key: &syn::Ident, locale_field: &syn::Ident) -> TokenStream {
        let Self(width, code) = self;
        quote!(__l_i18n_crate::__private::format_currency_to_display(#locale_field, #key, #width, #code))
    }

    fn to_fmt(&self, key: &Key, locale_field: &Key) -> TokenStream {
        let Self(width, code) = self;
        quote!(__l_i18n_crate::__private::format_currency_to_formatter(__formatter, *#locale_field, core::clone::Clone::clone(#key), #width, #code))
    }
}
...
impl ToTokens for CurrencyCode {
    fn to_token_stream(&self) -> TokenStream {
        ...
        quote!(__l_i18n_crate::reexports::icu::currency::CurrencyCode(
            __l_i18n_crate::reexports::tinystr!(3, #code)
        ))
    }
...
impl_to_tokens!(
    CurrencyWidth,
    __l_i18n_crate::reexports::icu::currency::options::Width,
    {
        Short,
        Narrow
    }
