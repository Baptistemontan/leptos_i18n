...
impl FormatterToTokens for NumberFormatter {
    ...
    fn to_view(&self, key: &syn::Ident, locale_field: &syn::Ident) -> TokenStream {
        let Self(strat) = self;
        quote!(__l_i18n_crate::__private::format_number_to_view(#locale_field, #key, #strat))
    }
    ...
    fn to_impl_display(&self, key: &syn::Ident, locale_field: &syn::Ident) -> TokenStream {
        let Self(strat) = self;
        quote!(__l_i18n_crate::__private::format_number_to_display(#locale_field, #key, #strat))
    }
    ...
    fn to_fmt(
        ...
        let Self(strat) = self;
        quote!(__l_i18n_crate::__private::format_number_to_formatter(#formatter_ident, #locale_field, core::clone::Clone::clone(#key), #strat))
    }
}
...
impl_to_tokens!(
    GroupingStrategy,
    __l_i18n_crate::reexports::icu::decimal::options::GroupingStrategy,
    {
        Auto,
        Never,
        Always,
