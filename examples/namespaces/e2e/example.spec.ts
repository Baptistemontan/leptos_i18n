...
import i18nClEn from "#locales/en/change_locale.json";
...
async function check_counter(
  ...
  await expect(page.locator(LNG_BUTTON_XPATH)).toHaveText(
    i18n.t("change_locale.click_to_change_lang")
  );
  await expect(page.locator(INC_BUTTON_XPATH)).toHaveText(
    i18n.t("counter.click_to_inc")
  );

  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("counter.click_count", { count: 0 })
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("counter.click_count", { count: 1 })
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("counter.click_count", { count: 2 })
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("counter.click_count", { count: 3 })
  );
}
...
async function check_state_keeping(page: Page, i18n: I18n) {
  ...
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("counter.click_count", { count: 4 })
  );
}
