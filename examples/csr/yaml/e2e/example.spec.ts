...
const i18nEn = YAML.parse(fs.readFileSync("./locales/en.yaml", "utf8"));
const i18nFr = YAML.parse(fs.readFileSync("./locales/fr.yaml", "utf8"));
...
async function check_counter(
  ...
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("click_count", {}, {}, 0)
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("click_count", {}, {}, 1)
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("click_count", {}, {}, 2)
  );
  await page.locator(INC_BUTTON_XPATH).click();
  await expect(page.locator(COUNTER_XPATH)).toHaveText(
    i18n.t("click_count", {}, {}, 3)
  );
}
