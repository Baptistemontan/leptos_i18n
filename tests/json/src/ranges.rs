...
fn u32_range_string() {
    // count = 0
    let count = 0;
    let en = td_string!(Locale::en, u32_range, count = count);
    assert_eq!(en.to_string(), "0");
    let fr = td_string!(Locale::fr, u32_range, count = count);
...
fn f32_or_range_string() {
    ...
    for count in [0.0, 5.0] {
        let en = td_string!(Locale::en, f32_OR_range, count = count);
        assert_eq!(en, "0 or 5");
        let fr = td_string!(Locale::fr, f32_OR_range, count = count);
        assert_eq!(fr, "0 or 5");
    }
...
fn args_to_range() {
    let count = move || 1;
    let en = td!(Locale::en, args_to_range, count = count);
    assert_eq_rendered!(en, "en 1");
    let fr = td!(Locale::fr, args_to_range, count = count);
