...
fn foreign_key_to_string() {
    let en = tdbg!(Locale::en, foreign_key_to_string);
    assert_eq!(en, "before Click to increment the counter after");
    let fr = tdbg!(Locale::fr, foreign_key_to_string);
...
fn foreign_key_to_interpolation() {
    for count in -5..5 {
        let en = tdbg!(Locale::en, foreign_key_to_interpolation, count);
        assert_eq_rendered!(en, format!("before You clicked {} times after", count));
        let fr = tdbg!(Locale::fr, foreign_key_to_interpolation, count);
