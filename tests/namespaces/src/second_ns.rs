...
#[test]
fn common_key() {
    let en = tdbg!(Locale::en, second_namespace.common_key);
    assert_eq!(en, "second namespace");
    let fr = tdbg!(Locale::fr, second_namespace.common_key);
...
fn click_count() {
    for i in -5..=5 {
        let count = move || i;
        let en = tdbg!(Locale::en, second_namespace.click_count, count);
        assert_eq_rendered!(en, format!("You clicked {} times", i));
        let fr = tdbg!(Locale::fr, second_namespace.click_count, count);
...
#[test]
fn click_to_inc() {
    let en = tdbg!(Locale::en, second_namespace.click_to_inc);
    assert_eq!(en, "Click to increment the counter");
    let fr = tdbg!(Locale::fr, second_namespace.click_to_inc);
    assert_eq!(fr, "Cliquez pour incrémenter le compteur");
}

#[test]
fn subkey_1() {
    let en = tdbg!(Locale::en, second_namespace.subkeys.subkey_1);
    assert_eq!(en, "subkey_1");
    let fr = tdbg!(Locale::fr, second_namespace.subkeys.subkey_1);
...
fn subkey_2() {
    ...
    let en = tdbg!(Locale::en, second_namespace.subkeys.subkey_2, <b>);
    assert_eq_rendered!(en, "<b>subkey_2</b>");
    let fr = tdbg!(Locale::fr, second_namespace.subkeys.subkey_2, <b>);
...
fn foreign_key_to_same_namespace() {
    let en = tdbg!(Locale::en, second_namespace.foreign_key_to_same_namespace);
    assert_eq!(en, "before second namespace after");
    let fr = tdbg!(Locale::fr, second_namespace.foreign_key_to_same_namespace);
